...
def score_sample_size(total_trades: int) -> int:
    ...
    if total_trades < 30:
        return 0
    if total_trades < 100:
        # Linear interpolation 8..15 for 30..99
        return 8 + int((total_trades - 30) / 70 * 7.499)
    if total_trades < 200:
        return 15 + int((total_trades - 100) / 100 * 4.999)
    return 20
...
def score_expectancy(win_rate: float, avg_win_pct: float, avg_loss_pct: float) -> int:
    ...
    if exp <= 0:
        return 0
    if exp < 0.5:
        return 5 + int(exp / 0.5 * 4.999)
    if exp < 1.5:
        return 10 + int((exp - 0.5) / 1.0 * 7.999)
    return 20
...
def score_risk_management(
    ...
    if max_drawdown_pct < 20:
        ...
        dd_score = int(12 * (50 - max_drawdown_pct) / 30.999)
...
def score_robustness(years_tested: int, num_parameters: int) -> int:
    ...
    if years_tested < 5:
        ...
        years_score = 5 + int((years_tested - 5) / 5 * 9.999)
